  ...
  <div class="relative flex min-h-screen items-center justify-center p-4">
    <div class="relative w-full max-w-sm">
      <div class="flex flex-col items-center mb-8">
        <svg class="size-12 text-ok mb-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
          <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
          <circle cx="12" cy="7" r="4" />
        </svg>
        <h1 class="text-2xl font-bold text-label">账号面板</h1>
        <p class="text-sm text-label2 mt-1">以账号身份登录（使用用户名/密码）</p>
      </div>

      <div class="card !p-6 w-full max-w-sm" style="box-shadow: var(--shadow-l)">
        <h2 class="text-lg font-semibold text-label mb-1">账号登录</h2>
        <p class="text-xs text-label2 mb-5">与"管理面板（API Key）"是独立凭据</p>
        ...
        <div class="mt-4 flex items-center justify-between text-xs">
          <router-link to="/login" class="text-label3 hover:text-label2 underline underline-offset-2">
            ← 返回 API Key 登录
          </router-link>
          <button class="text-label3 hover:text-label2 underline underline-offset-2" @click="onClear">
            清除本地会话
          </button>
        </div>
      </div>
