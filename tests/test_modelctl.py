...
def test_list_group_route_mapping(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setenv("LOG_DIR", str(tmp_path / "logs"))
    # 仅 qwen3.8-vllm 运行中 → 输入 qwen3.8 路由至它（状态列口径：is_running_any 统一判定）
...
def test_list_external_started_shown_as_running(tmp_path, monkeypatch, capsys):
...
def test_status_pid_residue_detected(tmp_path, monkeypatch, capsys):
    """PID 文件存在但 is_running_any False → 状态 'PID 残留'（venv 孤儿 + docker 切换残留）。"""
    ...
    (tmp_path / "cache" / "p-vllm.pid").write_text("999999", encoding="utf-8")  # dead PID
    monkeypatch.setattr("modelctl.cli.is_running_any", lambda name, profile: False)
    rc = cli.main(["status", "p-vllm", "--models-dir", str(tmp_path)])
    out = capsys.readouterr().out
    assert rc == 0
    assert "PID 残留" in out
