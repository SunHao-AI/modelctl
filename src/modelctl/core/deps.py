...
- --no-deps：对已声明的显式依赖（fastapi 等）可加；对第三方依赖不强制
  （让单包安装器拉取其自身的子依赖，否则 import 仍会失败）
"""
...
# 如 PyYAML 的顶层 import 是 yaml，快可使用 `uv pip install "PyYAML>=6.0"` 安装它）。
# - "core"：CLI 入口自身必需（loguru/yaml），缺失意味着整个工具不可用
# - "gateway"：modelctl core.gateway 运行所需（fastapi/uvicorn/httpx）
# - "stats"：modelctl core.stats 运行所需（纯 stdlib，理论无缺失）
...
def _pip_available() -> bool:
    """当前解释器内 pip 是否可用（uv 创建的 venv 默认无 pip）。"""
...
def _install_with_pip(req: str) -> bool:
    ...
    return (
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "-i", ALIYUN_PYPI_MIRROR,
             "--trusted-host", ALIYUN_TRUSTED_HOST, req],
...
def _install_requirement(req: str) -> bool:
    """单包安装：先用 uv（如可用），失败回退 pip；任一成功后即返回 True，都失败回 False。"""
    if _find_uv() is not None:
        if _install_with_uv(req):
            return True
        logger.warning(f"uv 安装 {req} 失败（镜像），回退 pip ...")
    if _pip_available():
